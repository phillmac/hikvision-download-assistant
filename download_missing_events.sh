...
while read -r mediatype eventtype fname curlcmd
do
    if [[ "${mediatype}" == "VIDEO" && "${eventtype}" != "MOTION" ]]
    then
        ((tcount=tcount+1))
...
done < <(jq -r  '. | "\(.mediaType) \(.eventType) \(.outputFilename) \(.curlCommand)"'  < results.json)
